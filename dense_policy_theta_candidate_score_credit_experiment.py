...
THETA_CANDIDATE_SCORE_CREDIT_MODE_DEFINITIONS = {
    "hierarchical_actor_true_conditional_route_policy": (
        "Control group: true two-level conditional policy with the current theta and route credit path."
    ),
    "hierarchical_actor_theta_candidate_score_credit": (
        "Test group: keep the same true conditional route policy and route branch untouched, "
        "but replace the theta effective advantage entering PPO with local-vs-best-offload "
        "candidate-score residual credit built from existing path costs."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for true conditional route policy vs theta candidate-score residual credit."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_theta_candidate_score_credit_compare(
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("theta_candidate_score_mean", "Theta Candidate Score Mean"),
            ("theta_candidate_score_std", "Theta Candidate Score Std"),
            ("theta_selected_score_mean", "Theta Selected Score Mean"),
            ("theta_expected_score_mean", "Theta Expected Score Mean"),
            ("theta_residual_credit_mean", "Theta Residual Credit Mean"),
            ("theta_residual_credit_std", "Theta Residual Credit Std"),
        ],
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_theta_candidate_score_credit_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "theta_candidate_score_credit_curve_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[("theta_approx_kl", "Theta Approx KL")],
        output_path=plots_dir / "theta_kl_curve_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[("theta_clip_fraction", "Theta Clip Fraction")],
        output_path=plots_dir / "theta_clip_curve_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[("offload_decision_agreement_ratio", "Offload Decision Agreement")],
        output_path=plots_dir / "offload_decision_agreement_curve_compare.png",
    )


def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_theta_candidate_score_credit_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "theta_score_source": (
                ...
                "best_offload_score = max(-block_path_cost_bs_now)"
            ),
            "theta_score_normalization": (
            ...
            json.dumps(
                {
                    ...
                    "theta_candidate_score_credit_mode_definition": (
                        THETA_CANDIDATE_SCORE_CREDIT_MODE_DEFINITIONS.get(
                            policy_ratio_mode,
                            "",
                        )
                    ),
                    ...
                    "theta_score_source_definition": (
                        "For each block: local_score = -block_path_cost_local_now; "
                        "best_offload_score = max(-block_path_cost_bs_now[..., bs])."
                    ),
                    "theta_score_normalization_definition": (
                        "For each sample/block over {local, best_offload}: "
                        "score_norm = (score - mean_candidate(score)) / "
                        "(std_candidate(score) + 1e-6)."
                    ),
                    "theta_residual_credit_definition": (
                        "selected_score = score_norm[selected_theta_action]; "
                        "expected_score = pi_old(local|s)*score_norm_local + "
                        "pi_old(offload|s)*score_norm_best_offload; "
                        "theta_effective_advantage = selected_score - expected_score."
                    ),
                    "route_branch_definition": (
                        "Route branch and route surrogate remain exactly the same as true conditional route policy."
                    ),
        ...
        manifest["runs"].append(
            {
                ...
                "theta_candidate_score_credit_stats": str(
    ...
    summary_csv = root_dir / "policy_theta_candidate_score_credit_summary.csv"
    summary_json = root_dir / "policy_theta_candidate_score_credit_summary.json"
    summary_md = root_dir / "policy_theta_candidate_score_credit_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy theta candidate-score credit experiment completed. "
