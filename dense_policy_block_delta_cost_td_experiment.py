...
BLOCK_LOCAL_COST_DEFINITION = (
    "local_cost_b = workload_b + virtual_queue_b + local_queue_b + access_queue_sensor(b)."
)
BLOCK_TD_MODE_DEFINITIONS = {
    "blockwise_td_advantage_surrogate_mean": (
        "local_cost_b_now = workload_b(now) + virtual_queue_b(now) + local_queue_b(now) + "
        "access_queue_sensor(b)(now); r_b = -local_cost_b_now; "
        "A_b = r_b + gamma * (1 - done) * V_b(s_next) - V_b(s)."
    ),
    "blockwise_delta_cost_td_advantage_surrogate_mean": (
        "local_cost_b_now = workload_b(now) + virtual_queue_b(now) + local_queue_b(now) + "
        "access_queue_sensor(b)(now); "
        "local_cost_b_next = workload_b(next) + virtual_queue_b(next) + local_queue_b(next) + "
        "access_queue_sensor(b)(next); "
        "delta_cost_b = local_cost_b_next - local_cost_b_now; "
        "r_b = -delta_cost_b = local_cost_b_now - local_cost_b_next; "
        "A_b = r_b + gamma * (1 - done) * V_b(s_next) - V_b(s)."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for absolute-cost TD vs delta-cost TD block advantages."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_block_td_value_compare(
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("block_value_pred_mean", "Block Value Pred Mean"),
...
def plot_block_delta_cost_compare(
    mode_logs: dict[str, pd.DataFrame],
    output_path: Path,
) -> None:
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("block_local_cost_now_mean", "Block Local Cost Now Mean"),
            ("block_local_cost_next_mean", "Block Local Cost Next Mean"),
            ("block_delta_cost_mean", "Block Delta Cost Mean"),
            ("block_td_reward_mean", "Block TD Reward Mean"),
        ],
        output_path=output_path,
    )
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_block_delta_cost_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "block_delta_cost_curve_compare.png",
    )
    plot_metric_compare(
        mode_logs,
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_block_delta_cost_td_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "block_local_cost_definition": BLOCK_LOCAL_COST_DEFINITION,
            "block_td_mode_definitions": BLOCK_TD_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "block_local_cost_definition": BLOCK_LOCAL_COST_DEFINITION,
                    "block_td_mode_definition": BLOCK_TD_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "block_td_mode_definitions": BLOCK_TD_MODE_DEFINITIONS,
        ...
        manifest["runs"].append(
            {
                ...
                "block_delta_cost_stats": str(run_dir / "block_delta_cost_stats_by_epoch.csv"),
            }
        )

    summary_df = pd.DataFrame(all_metrics)
    summary_csv = root_dir / "policy_block_delta_cost_td_summary.csv"
    summary_json = root_dir / "policy_block_delta_cost_td_summary.json"
    summary_md = root_dir / "policy_block_delta_cost_td_summary.md"
    ...
    manifest["summary"] = {
        ...
        "block_delta_cost_curve_compare": str(
            root_dir / "comparison_plots" / "block_delta_cost_curve_compare.png"
        ),
...
def main() -> None:
    ...
    print(
        "Dense policy block delta-cost TD experiment completed. "
