...
DEFAULT_REWARD_MODES = ("raw_total", "per_sensor")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense reward-mode experiment comparing raw_total and per_sensor."
    )
    parser.add_argument(
        "--reward-modes",
        nargs="+",
...
def build_reward_mode_config(reward_mode: str, seed: int) -> MECConfig:
...
def run_training_epoch(
    ...
    episode_raw_reward_delay_term = 0.0
    episode_raw_reward_energy_term = 0.0
    episode_raw_reward_backlog_term = 0.0
    ...
    for _ in range(config.training.time_steps):
        ...
        episode_raw_reward_delay_term += float(info["raw_reward_delay_term"])
        episode_raw_reward_energy_term += float(info["raw_reward_energy_term"])
        episode_raw_reward_backlog_term += float(info["raw_reward_backlog_term"])
    ...
    return {
        ...
        "episode_raw_reward_delay_term": episode_raw_reward_delay_term,
        "episode_raw_reward_energy_term": episode_raw_reward_energy_term,
        "episode_raw_reward_backlog_term": episode_raw_reward_backlog_term,
...
def extract_metrics(run_dir: Path, reward_mode: str) -> dict[str, float | int | str]:
    ...
    metrics: dict[str, float | int | str] = {
        ...
        "final_reward_delay_term": float(logs["episode_reward_delay_term"].iloc[-1]),
        "final_reward_energy_term": float(logs["episode_reward_energy_term"].iloc[-1]),
        "final_reward_backlog_term": float(logs["episode_reward_backlog_term"].iloc[-1]),
...
def plot_comparisons(root_dir: Path, ranked_df: pd.DataFrame) -> None:
    ...
    for row in ranked_df.itertuples(index=False):
        mode_logs[str(row.reward_mode)] = pd.read_csv(Path(row.run_dir) / "train_logs.csv")

    plt.figure(figsize=(8, 5))
    for reward_mode, logs in mode_logs.items():
        plt.plot(logs["epoch"], logs["critic_loss"], marker="o", label=reward_mode)
    plt.xlabel("epoch")
    plt.ylabel("critic_loss")
    plt.title("Critic Loss by Reward Mode")
    plt.legend()
    plt.tight_layout()
    ...
    reward_columns = [
        ("episode_reward_delay_term", "Reward Delay Term"),
        ("episode_reward_energy_term", "Reward Energy Term"),
        ("episode_reward_backlog_term", "Reward Backlog Term"),
    ]
    for axis, (column, title) in zip(axes, reward_columns):
        for reward_mode, logs in mode_logs.items():
            axis.plot(logs["epoch"], logs[column], marker="o", label=reward_mode)
        axis.set_ylabel(column)
        axis.set_title(title)
        axis.legend()
    axes[-1].set_xlabel("epoch")
    fig.tight_layout()
    fig.savefig(plots_dir / "reward_terms_compare.png", dpi=150)
    plt.close(fig)


def run_experiment(reward_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime("dense_reward_mode_experiment_%Y%m%d_%H%M%S")
    ...
    for reward_mode in reward_modes:
        ...
        run_dir = root_dir / f"reward_mode_{reward_mode}_{run_tag}"
        run_dir.mkdir(parents=True, exist_ok=True)

        config = build_reward_mode_config(reward_mode=reward_mode, seed=seed)
        ...
        print(f"[train] reward_mode={reward_mode}, seed={seed} -> {run_dir}")
        ...
        metrics = extract_metrics(run_dir=run_dir, reward_mode=reward_mode)
    ...
    summary_csv = root_dir / "reward_mode_summary.csv"
    summary_json = root_dir / "reward_mode_summary.json"
    summary_md = root_dir / "reward_mode_summary.md"
    ...
    manifest["summary"] = {
        ...
        "reward_terms_plot": str(root_dir / "comparison_plots" / "reward_terms_compare.png"),
    }
...
def main() -> None:
    ...
    print(f"Dense reward-mode experiment completed. Results saved to: {root_dir}")
