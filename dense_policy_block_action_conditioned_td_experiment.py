...
ACTION_CONDITIONED_TD_MODE_DEFINITIONS = {
    "blockwise_td_advantage_surrogate_mean": (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for baseline block TD vs action-conditioned block TD."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_block_action_conditioned_cost_compare(
    mode_logs: dict[str, pd.DataFrame],
    output_path: Path,
) -> None:
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("block_path_cost_local_mean", "Block Path Cost Local Mean"),
            ("block_path_cost_bs1_mean", "Block Path Cost BS1 Mean"),
            ("block_path_cost_bs2_mean", "Block Path Cost BS2 Mean"),
            ("block_action_conditioned_cost_now_mean", "Action-Conditioned Cost Now Mean"),
            ("block_action_conditioned_cost_next_mean", "Action-Conditioned Cost Next Mean"),
            ("block_delta_cost_mean", "Action-Conditioned Delta Cost Mean"),
        ],
        output_path=output_path,
    )
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_block_action_conditioned_cost_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "block_action_conditioned_cost_curve_compare.png",
    )
    plot_bucket_metric_compare(
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_block_action_conditioned_td_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "action_conditioned_td_mode_definitions": ACTION_CONDITIONED_TD_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "block_path_cost_definition": BLOCK_PATH_COST_DEFINITION,
                    "action_conditioned_td_mode_definition": ACTION_CONDITIONED_TD_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "action_conditioned_td_mode_definitions": ACTION_CONDITIONED_TD_MODE_DEFINITIONS,
        ...
        manifest["runs"].append(
            {
                ...
                "block_action_conditioned_cost_stats": str(
    ...
    summary_csv = root_dir / "policy_block_action_conditioned_td_summary.csv"
    summary_json = root_dir / "policy_block_action_conditioned_td_summary.json"
    summary_md = root_dir / "policy_block_action_conditioned_td_summary.md"
    ...
    manifest["summary"] = {
        ...
        "block_action_conditioned_cost_curve_compare": str(
            root_dir / "comparison_plots" / "block_action_conditioned_cost_curve_compare.png"
        ),
...
def main() -> None:
    ...
    print(
        "Dense policy block action-conditioned TD experiment completed. "
