...
ALTERNATING_BRANCH_MODE_DEFINITIONS = {
    "hierarchical_actor_branchwise_balanced_pg": (
        "Control group: separate theta / route backbones with hard route mask and "
        "branchwise normalized/balanced theta-route surrogates, but both branches are still "
        "optimized in the same actor optimizer step."
    ),
    "hierarchical_actor_alternating_branch_pg": (
        "Test group: keep the same separate backbones, hard route mask, A_theta/A_route, and "
        "branchwise normalization, but split each PPO actor update into theta-only step first "
        "and route-only step second on the same rollout batch."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for joint branchwise-balanced updates vs alternating "
            "theta-first / route-second branch updates."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_alternating_step_kl_compare(
    mode_logs: dict[str, pd.DataFrame],
    output_path: Path,
) -> None:
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("theta_only_step_kl", "Theta-Only Step KL"),
            ("route_only_step_kl", "Route-Only Step KL"),
            ("theta_only_step_prob_gain", "Theta-Only Step Prob Gain"),
            ("route_only_step_prob_gain", "Route-Only Step Prob Gain"),
            ("route_after_theta_shift", "Route Shift After Theta Step"),
            ("theta_after_route_shift", "Theta Shift After Route Step"),
        ],
        output_path=output_path,
    )
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Alternating Branch PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Alternating Branch PG Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Alternating Branch PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Alternating Branch PG Mode",
        ylabel="approx_kl",
    ...
    plot_alternating_step_kl_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "alternating_step_kl_curve_compare.png",
    )
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_alternating_branch_pg_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "alternating_branch_mode_definitions": ALTERNATING_BRANCH_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "alternating_branch_mode_definition": ALTERNATING_BRANCH_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "alternating_branch_mode_definitions": ALTERNATING_BRANCH_MODE_DEFINITIONS,
                    ...
                    "joint_actor_loss_definition": (
                        "control only: actor_loss = 0.5 * theta_loss + 0.5 * route_loss"
                    ),
                    "theta_only_update_definition": (
                        "theta-only step: optimize theta_loss with theta branch parameters only on "
                        "the same rollout mini-batch."
                    ),
                    "route_only_update_definition": (
                        "route-only step: optimize route_loss with route branch parameters only on "
                        "the same rollout mini-batch and active route blocks only."
                    ),
                    "update_order_definition": "theta-first, then route-second within each PPO minibatch",
                    "rollout_reuse_definition": (
                        "Both branch steps use the same rollout batch and the same old action/log-prob cache."
                    ),
        ...
        manifest["runs"].append(
            {
                ...
                "branchwise_balanced_pg_stats": str(
    ...
    summary_csv = root_dir / "policy_alternating_branch_pg_summary.csv"
    summary_json = root_dir / "policy_alternating_branch_pg_summary.json"
    summary_md = root_dir / "policy_alternating_branch_pg_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy alternating-branch PG experiment completed. "
