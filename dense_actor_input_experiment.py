...
DEFAULT_ACTOR_INPUT_MODES = (
    "current_actor_input",
    "normalized_augmented_actor_input",
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense actor input comparison on the fixed healthy critic mainline."
    )
    parser.add_argument(
        "--actor-input-modes",
        nargs="+",
        default=list(DEFAULT_ACTOR_INPUT_MODES),
        help="Actor input modes to compare.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def build_actor_input_config(actor_input_mode: str, seed: int) -> MECConfig:
    ...
    ppo = replace(
        config.ppo,
        actor_input_mode=actor_input_mode,
...
def build_state_layout(config: MECConfig) -> dict[str, int]:
    sensor_count = len(config.sensors)
    task_count = len(config.tasks)
    base_station_count = len(config.base_stations)
    access_queue_count = len(config.access_links)
    sensor_task_count = sensor_count * task_count
    compute_rate_count = sum(
        len(base_station.compute_units) * task_count for base_station in config.base_stations
    )
    return {
        "sensor_count": sensor_count,
        "task_count": task_count,
        "base_station_count": base_station_count,
        "sensor_task_count": sensor_task_count,
        "access_queue_count": access_queue_count,
        "virtual_queue_count": access_queue_count * task_count,
        "bs_queue_count": base_station_count * task_count,
        "local_queue_count": sensor_task_count,
        "reachable_mask_count": sensor_count * base_station_count,
        "access_gain_count": sensor_count,
        "compute_rate_count": compute_rate_count,
    }
...
def rank_runs(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    ranked = ranked.sort_values(
        by=[
            ...
            "final_reward",
            "reward_gap_abs",
            "final_actor_input_dim_std_mean",
            "final_actor_input_clip_fraction",
...
def load_mode_logs(ranked_df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    for row in ranked_df.itertuples(index=False):
        mode_logs[str(row.actor_input_mode)] = pd.read_csv(Path(row.run_dir) / "train_logs.csv")
    return mode_logs
...
def plot_metric_compare(
    ...
    for actor_input_mode, logs in mode_logs.items():
        values = logs[column] if column in logs.columns else np.zeros(len(logs))
        plt.plot(logs["epoch"], values, marker="o", label=actor_input_mode)
    plt.xlabel("epoch")
    plt.ylabel(ylabel)
    plt.title(title)
    plt.legend()
...
def plot_two_panel_compare(
    ...
    for axis, (column, title) in zip(axes, panel_specs, strict=True):
        for actor_input_mode, logs in mode_logs.items():
            values = logs[column] if column in logs.columns else np.zeros(len(logs))
            axis.plot(logs["epoch"], values, marker="o", label=actor_input_mode)
...
def plot_action_distribution_compare(
    ...
    for actor_input_mode, logs in mode_logs.items():
        action_prob_std = logs["action_prob_std"] if "action_prob_std" in logs.columns else np.zeros(len(logs))
        policy_confidence = (
            logs["policy_confidence_mean"]
            if "policy_confidence_mean" in logs.columns
            else np.zeros(len(logs))
        )
        axes[0].plot(logs["epoch"], action_prob_std, marker="o", label=actor_input_mode)
        axes[1].plot(logs["epoch"], policy_confidence, marker="o", label=actor_input_mode)
    ...
    for idx, (actor_input_mode, logs) in enumerate(mode_logs.items()):
        serialized = (
...
def plot_scatter_compare(ranked_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for axis, row in zip(axes[0], ranked_df.itertuples(index=False), strict=True):
        ...
        axis.set_title(str(row.actor_input_mode))
...
def plot_bucket_compare(root_dir: Path, ranked_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for axis, row in zip(axes[0], ranked_df.itertuples(index=False), strict=True):
        ...
        bucket_summary.insert(0, "actor_input_mode", str(row.actor_input_mode))
...
def plot_comparisons(root_dir: Path, ranked_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        mode_logs,
        column="critic_loss",
        title="Critic Loss by Actor Input Mode",
        ylabel="critic_loss",
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Actor Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Explained Variance by Actor Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Prediction/Target Correlation by Actor Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Prediction Std over Target Std by Actor Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Probe Prediction Std by Actor Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Critic Head Bias Mean by Actor Input Mode",
        ylabel="critic_head_bias_mean",
        output_path=plots_dir / "critic_head_bias_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="critic_input_std",
        title="Critic Input Std by Actor Input Mode",
        ylabel="critic_input_std",
        output_path=plots_dir / "critic_input_std_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("critic_input_dim_std_mean", "Critic Input Dim Std Mean"),
            ("critic_input_dim_std_min", "Critic Input Dim Std Min"),
        ],
        output_path=plots_dir / "critic_input_dim_std_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="critic_input_clip_fraction",
        title="Critic Input Clip Fraction by Actor Input Mode",
        ylabel="critic_input_clip_fraction",
        output_path=plots_dir / "critic_input_clip_fraction_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="actor_input_std",
        title="Actor Input Std by Actor Input Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="actor_input_clip_fraction",
        title="Actor Input Clip Fraction by Actor Input Mode",
        ylabel="actor_input_clip_fraction",
        output_path=plots_dir / "actor_input_clip_fraction_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="actor_grad_norm",
        title="Actor Grad Norm by Actor Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Policy Entropy by Actor Input Mode",
...
def run_experiment(actor_input_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime("dense_actor_input_experiment_%Y%m%d_%H%M%S")
    ...
    probe_config = build_actor_input_config(actor_input_mode=actor_input_modes[0], seed=seed)
    ...
    manifest: dict[str, object] = {
        ...
        "actor_input_modes": actor_input_modes,
        "runs": [],
    }

    all_metrics: list[dict[str, float | int | str]] = []
    for actor_input_mode in actor_input_modes:
        run_tag = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        run_dir = root_dir / f"actor_input_{actor_input_mode}_{run_tag}"
        run_dir.mkdir(parents=True, exist_ok=True)

        config = build_actor_input_config(actor_input_mode=actor_input_mode, seed=seed)
        ...
        print(f"[train] actor_input_mode={actor_input_mode}, seed={seed} -> {run_dir}")
    ...
    summary_csv = root_dir / "actor_input_summary.csv"
    summary_json = root_dir / "actor_input_summary.json"
    summary_md = root_dir / "actor_input_summary.md"
    ...
    manifest["summary"] = {
        ...
        "actor_input_std_plot": str(
            root_dir / "comparison_plots" / "actor_input_std_compare.png"
        ),
        "actor_input_dim_std_plot": str(
            root_dir / "comparison_plots" / "actor_input_dim_std_compare.png"
        ),
        "actor_grad_norm_plot": str(
            root_dir / "comparison_plots" / "actor_grad_norm_compare.png"
        ),
        "policy_entropy_plot": str(
            root_dir / "comparison_plots" / "policy_entropy_compare.png"
        ),
...
def main() -> None:
    ...
    root_dir = run_experiment(
        actor_input_modes=args.actor_input_modes,
        seed=args.seed,
        output_root=Path(args.output_root),
    )
    print(f"Dense actor input experiment completed. Results saved to: {root_dir}")
