...
DEFAULT_CRITIC_INPUT_MODES = (
    "current_stronger_input",
    "normalized_augmented_critic_input",
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense critic input comparison with stronger critic backbone."
    )
    parser.add_argument(
        "--critic-input-modes",
        nargs="+",
        default=list(DEFAULT_CRITIC_INPUT_MODES),
        help="Critic input modes to compare.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def build_critic_input_config(critic_input_mode: str, seed: int) -> MECConfig:
...
def build_critic_state_layout(config: MECConfig) -> dict[str, int]:
...
def rank_runs(summary_df: pd.DataFrame) -> pd.DataFrame:
    ...
    ranked = ranked.sort_values(
        by=[
            ...
            "final_target_bucket_prediction_slope",
...
def load_mode_logs(ranked_df: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    for row in ranked_df.itertuples(index=False):
        mode_logs[str(row.critic_input_mode)] = pd.read_csv(Path(row.run_dir) / "train_logs.csv")
    return mode_logs
...
def plot_metric_compare(
    ...
    for critic_input_mode, logs in mode_logs.items():
        plt.plot(logs["epoch"], logs[column], marker="o", label=critic_input_mode)
    plt.xlabel("epoch")
    plt.ylabel(ylabel)
    plt.title(title)
    plt.legend()
...
def plot_two_panel_compare(
    ...
    for axis, (column, title) in zip(axes, panel_specs, strict=True):
        for critic_input_mode, logs in mode_logs.items():
            axis.plot(logs["epoch"], logs[column], marker="o", label=critic_input_mode)
...
def plot_scatter_compare(ranked_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for axis, row in zip(axes[0], ranked_df.itertuples(index=False), strict=True):
        ...
        axis.set_title(str(row.critic_input_mode))
...
def plot_bucket_compare(root_dir: Path, ranked_df: pd.DataFrame, output_path: Path) -> None:
    ...
    for axis, row in zip(axes[0], ranked_df.itertuples(index=False), strict=True):
        ...
        bucket_summary.insert(0, "critic_input_mode", str(row.critic_input_mode))
...
def plot_comparisons(root_dir: Path, ranked_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        mode_logs,
        column="critic_loss",
        title="Critic Loss by Critic Input Mode",
        ylabel="critic_loss",
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Explained Variance by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Prediction/Target Correlation by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Prediction Std over Target Std by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Probe Prediction Std by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Critic Head Bias Mean by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Critic Input Std by Critic Input Mode",
    ...
    plot_metric_compare(
        ...
        title="Critic Input Clip Fraction by Critic Input Mode",
        ylabel="critic_input_clip_fraction",
        output_path=plots_dir / "critic_input_clip_fraction_compare.png",
    )


def run_experiment(critic_input_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime("dense_critic_input_experiment_%Y%m%d_%H%M%S")
    ...
    probe_config = build_critic_input_config(critic_input_mode=critic_input_modes[0], seed=seed)
    ...
    manifest: dict[str, object] = {
        ...
        "critic_input_modes": critic_input_modes,
        "runs": [],
    }

    all_metrics: list[dict[str, float | int | str]] = []
    for critic_input_mode in critic_input_modes:
        run_tag = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        run_dir = root_dir / f"critic_input_{critic_input_mode}_{run_tag}"
        run_dir.mkdir(parents=True, exist_ok=True)

        config = build_critic_input_config(critic_input_mode=critic_input_mode, seed=seed)
        critic_state_layout = build_critic_state_layout(config)
            ...
            json.dumps(
                {
                    ...
                    "critic_state_layout": critic_state_layout,
        ...
        print(f"[train] critic_input_mode={critic_input_mode}, seed={seed} -> {run_dir}")
        ...
        train_with_diagnosis(
            ...
            agent_kwargs={"critic_state_layout": critic_state_layout},
        )
    ...
    summary_csv = root_dir / "critic_input_summary.csv"
    summary_json = root_dir / "critic_input_summary.json"
    summary_md = root_dir / "critic_input_summary.md"
    ...
    manifest["summary"] = {
        ...
        "critic_input_std_plot": str(
            root_dir / "comparison_plots" / "critic_input_std_compare.png"
        ),
        "critic_input_dim_std_plot": str(
            root_dir / "comparison_plots" / "critic_input_dim_std_compare.png"
        ),
        "critic_input_clip_fraction_plot": str(
            root_dir / "comparison_plots" / "critic_input_clip_fraction_compare.png"
        ),
    }
...
def main() -> None:
    ...
    root_dir = run_experiment(
        critic_input_modes=args.critic_input_modes,
        seed=args.seed,
        output_root=Path(args.output_root),
    )
    print(f"Dense critic input experiment completed. Results saved to: {root_dir}")
