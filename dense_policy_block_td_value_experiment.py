...
BLOCK_TD_VALUE_DEFINITION = (
    "For blockwise_td_advantage_surrogate_mean: reuse the lightweight 24-block value head on top "
    "of the existing critic backbone as block value predictor V_b(s). "
    "Define local_cost_b = workload_b + virtual_queue_b + local_queue_b + access_queue_sensor(b), "
    "all from the current raw state at the current timestep; r_b = -local_cost_b. "
    "Then A_b = r_b + gamma * (1 - done) * V_b(s_next) - V_b(s). "
    "The block value head is trained with the configured value loss against this block TD target, "
    "using detached critic-backbone features so the global critic mainline remains intact."
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for block-specific TD-style value signals."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_block_td_value_compare(
    ...
    plot_two_panel_compare(
        ...
        panel_specs=[
            ...
            ("block_value_pred_std", "Block Value Pred Std"),
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_block_td_value_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "block_td_value_definition": BLOCK_TD_VALUE_DEFINITION,
    ...
    summary_csv = root_dir / "policy_block_td_value_summary.csv"
    summary_json = root_dir / "policy_block_td_value_summary.json"
    summary_md = root_dir / "policy_block_td_value_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy block TD value experiment completed. "
