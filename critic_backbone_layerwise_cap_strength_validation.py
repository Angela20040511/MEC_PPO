...
VALIDATION_STEM = "critic_backbone_layerwise_cap_strength_validation"
...
STRONGER_0W_LAYERWISE_CAPS = {
    "critic_backbone.0.weight": 1e3,
    "critic_backbone.6.weight": 4e3,
    "critic_backbone.7.weight": 4e3,
    "critic_backbone.4.bias": 4e3,
}
GROUP_SPECS: dict[str, dict[str, Any]] = {
    ...
    "critic_backbone_layerwise_preconditioner_clip_stronger_0w_head_adam": {
        "backbone_preconditioner_cap": None,
        "layerwise_backbone_preconditioner_caps": STRONGER_0W_LAYERWISE_CAPS,
        "clip_focus_parameter_names": HOTSPOT_CLIP_LAYERS,
        "description": (
            "Only strengthen the 0.weight hotspot cap from 2e3 to 1e3 while "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Formal short validation for critic_backbone layerwise cap strength, "
            "focused on stronger clipping of critic_backbone.0.weight."
        )
    )
...
def main() -> None:
    ...
    print(
        "Critic backbone layerwise cap strength validation completed. "
