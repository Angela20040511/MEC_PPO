...
FIXED_NOMA_QUANTILE = 0.85
FIXED_MAX_CLUSTER_SIZE = 3

DEFAULT_ALPHA_P_GRID = (0.2, 0.4, 0.6, 0.8)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled alpha_p search under fixed PPO, DT-B, and scheduler baselines."
    )
    parser.add_argument(
        "--alpha-p-grid",
        type=float,
...
def build_alpha_p_config(alpha_p: float, seed: int):
...
def extract_metrics(run_dir: Path, alpha_p: float, seed: int) -> dict[str, float | int | str]:
...
def run_search(alpha_p_grid: list[float], seed: int, output_root: Path) -> Path:
    search_tag = datetime.now().strftime("fast_scheduler_alpha_p_search_%Y%m%d_%H%M%S")
    ...
    for alpha_p in alpha_p_grid:
        ...
        config = build_alpha_p_config(alpha_p=alpha_p, seed=seed)
        ...
        print(f"[train] alpha_p={alpha_p:.2f}, seed={seed} -> {run_dir}")
        ...
        metrics = extract_metrics(run_dir=run_dir, alpha_p=alpha_p, seed=seed)
    ...
    summary_csv = root_dir / "fast_scheduler_alpha_p_summary.csv"
    summary_json = root_dir / "fast_scheduler_alpha_p_summary.json"
    summary_md = root_dir / "fast_scheduler_alpha_p_summary.md"
...
def main() -> None:
    args = parse_args()
    root_dir = run_search(alpha_p_grid=args.alpha_p_grid, seed=args.seed, output_root=Path(args.output_root))
    print(f"Fast scheduler alpha_p search completed. Results saved to: {root_dir}")
