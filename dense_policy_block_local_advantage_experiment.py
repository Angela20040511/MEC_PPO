...
BLOCK_ADVANTAGE_SCALE_DEFINITION = (
    "For blockwise_scaled_advantage_surrogate_mean: "
    "local_activity_b = workload_b + virtual_queue_b + local_queue_b + access_queue_sensor(b), "
    "all from the current normalized state at the same timestep; "
    "s_b = (local_activity_b + eps) / mean_blocks(local_activity + eps), "
    "so s_b is nonnegative and each sample has mean block scale = 1."
)
BLOCK_PSEUDOLOCAL_ADVANTAGE_DEFINITION = (
    "For blockwise_pseudolocal_advantage_surrogate_mean: "
    "local_activity_b = workload_b + virtual_queue_b + local_queue_b + access_queue_sensor(b); "
    "z_b = (local_activity_b - mean_blocks(local_activity)) / (std_blocks(local_activity) + eps); "
    "u_b = tanh(z_b); "
    "u_b = u_b / (mean_blocks(abs(u_b)) + eps); "
    "A_b = A * u_b. "
    "This uses only current-sample current-state information, produces signed block-specific credit, "
    "and keeps the mean absolute coefficient near 1 for stable scale."
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for block-specific pseudo-local advantages."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_block_advantage_compare(
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("block_advantage_mean", "Block Advantage Mean"),
            ("block_advantage_std", "Block Advantage Std"),
            ("block_advantage_max", "Block Advantage Max"),
            ("block_advantage_min", "Block Advantage Min"),
            ("block_advantage_entropy", "Block Advantage Entropy"),
            ("top_k_block_advantage_share", "Top-K Block Advantage Share"),
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_block_advantage_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "block_advantage_curve_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="block_advantage_entropy",
        title="Block Advantage Entropy by Policy Surrogate Mode",
        ylabel="block_advantage_entropy",
        output_path=plots_dir / "block_advantage_entropy_curve_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="top_k_block_advantage_share",
        title="Top-K Block Advantage Share by Policy Surrogate Mode",
        ylabel="top_k_block_advantage_share",
        output_path=plots_dir / "top_k_block_advantage_share_curve_compare.png",
    )
    plot_bucket_metric_compare(
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_block_local_advantage_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "block_advantage_scale_definition": BLOCK_ADVANTAGE_SCALE_DEFINITION,
            "block_pseudolocal_advantage_definition": BLOCK_PSEUDOLOCAL_ADVANTAGE_DEFINITION,
    ...
    summary_csv = root_dir / "policy_block_local_advantage_summary.csv"
    summary_json = root_dir / "policy_block_local_advantage_summary.json"
    summary_md = root_dir / "policy_block_local_advantage_summary.md"
    ...
    manifest["summary"] = {
        ...
        "block_advantage_entropy_curve_compare": str(
            root_dir / "comparison_plots" / "block_advantage_entropy_curve_compare.png"
        ),
        "top_k_block_advantage_share_curve_compare": str(
            root_dir / "comparison_plots" / "top_k_block_advantage_share_curve_compare.png"
        ),
...
def main() -> None:
    ...
    print(
        "Dense policy block local advantage experiment completed. "
