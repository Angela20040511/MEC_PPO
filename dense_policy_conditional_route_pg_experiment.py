...
CONDITIONAL_ROUTE_MODE_DEFINITIONS = {
    "hierarchical_actor_factorized_ratio_pg": (
        "Control group: fully factorized theta / route PPO objective with separate backbones, "
        "hard route mask, branchwise normalized advantages, and alternating theta-first then "
        "route-second updates, using raw route-logit Gaussian log-probs."
    ),
    "hierarchical_actor_factorized_conditional_route_pg": (
        "Test group: keep the same fully factorized theta / route PPO objective and alternating "
        "theta-first then route-second updates, but redefine route branch old/new log-probs from "
        "the semantic BS1-vs-BS2 route margin so the route objective matches conditional routing "
        "probabilities instead of raw two-logit translation-sensitive Gaussian density."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for factorized-ratio PG vs semantic conditional-route PG."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Conditional Route PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Conditional Route PG Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Conditional Route PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Conditional Route PG Mode",
        ylabel="approx_kl",
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_conditional_route_pg_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "conditional_route_mode_definitions": CONDITIONAL_ROUTE_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "conditional_route_mode_definition": CONDITIONAL_ROUTE_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "conditional_route_mode_definitions": CONDITIONAL_ROUTE_MODE_DEFINITIONS,
                    ...
                    "conditional_route_margin_definition": (
                        "For hierarchical_actor_factorized_conditional_route_pg only: "
                        "route_margin_b = route_logit_BS1_b - route_logit_BS2_b; "
                        "q_bs1_b = sigmoid(route_margin_b), q_bs2_b = 1 - q_bs1_b. "
                        "Route old/new logprob are defined from the scalar Gaussian density of "
                        "route_margin_b rather than the sum of raw two-logit Gaussian log-probs."
                    ),
    ...
    summary_csv = root_dir / "policy_conditional_route_pg_summary.csv"
    summary_json = root_dir / "policy_conditional_route_pg_summary.json"
    summary_md = root_dir / "policy_conditional_route_pg_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy conditional-route PG experiment completed. "
