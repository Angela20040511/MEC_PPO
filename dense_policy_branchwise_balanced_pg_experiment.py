...
BRANCHWISE_BALANCED_MODE_DEFINITIONS = {
    "hierarchical_actor_separate_theta_route_backbones": (
        "Control group: separate theta / route backbones with hard offload-masked route updates; "
        "theta and route surrogates still use current aggregation."
    ),
    "hierarchical_actor_branchwise_balanced_pg": (
        "Test group: same separate theta / route backbones and same hard offload mask, but "
        "theta advantages are normalized on all blocks, route advantages are normalized only on "
        "active route blocks, theta loss averages over all blocks, route loss averages over active "
        "route blocks, and final actor loss = 0.5 * theta_loss + 0.5 * route_loss."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for separate-backbone hierarchical actor vs "
            "branchwise normalized and balanced policy gradient."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_branchwise_loss_ratio_compare(
    mode_logs: dict[str, pd.DataFrame],
    output_path: Path,
) -> None:
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("theta_loss_mean", "Theta Loss Mean"),
            ("route_loss_mean", "Route Loss Mean"),
            ("theta_route_loss_ratio", "Route/Theta Loss Ratio"),
            ("theta_adv_norm_std", "Theta Adv Norm Std"),
            ("route_adv_norm_std", "Route Adv Norm Std"),
            ("route_adv_norm_used_fallback", "Route Norm Fallback"),
        ],
        output_path=output_path,
    )
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Branchwise Balanced PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Branchwise Balanced PG Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Branchwise Balanced PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Branchwise Balanced PG Mode",
        ylabel="approx_kl",
    ...
    plot_branchwise_loss_ratio_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "branchwise_loss_ratio_curve_compare.png",
    )
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_branchwise_balanced_pg_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "branchwise_balanced_mode_definitions": BRANCHWISE_BALANCED_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "branchwise_balanced_mode_definition": BRANCHWISE_BALANCED_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "branchwise_balanced_mode_definitions": BRANCHWISE_BALANCED_MODE_DEFINITIONS,
                    ...
                    "theta_advantage_normalization_definition": (
                        "A_theta_norm = (A_theta - mean(A_theta_all_blocks)) / (std(A_theta_all_blocks) + eps)"
                    ),
                    "route_advantage_normalization_definition": (
                        "A_route_norm = (A_route - mean(A_route_active_blocks)) / "
                        "(std(A_route_active_blocks) + eps); if active route count < 2 or std is too "
                        "small, fall back to unnormalized active A_route."
                    ),
                    ...
                    "actor_loss_definition": "actor_loss = 0.5 * theta_loss + 0.5 * route_loss",
    ...
    summary_csv = root_dir / "policy_branchwise_balanced_pg_summary.csv"
    summary_json = root_dir / "policy_branchwise_balanced_pg_summary.json"
    summary_md = root_dir / "policy_branchwise_balanced_pg_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy branchwise balanced-PG experiment completed. "
