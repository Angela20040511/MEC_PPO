...
VALIDATION_STEM = "critic_backbone_layerwise_cap_long_confirmation_validation"
...
GROUP_SPECS: dict[str, dict[str, Any]] = {
    ...
    "critic_backbone_layerwise_preconditioner_clip_head_adam": {
        ...
        "description": (
            "Long confirmation old candidate: 0.weight=2e3, "
    ...
    "critic_backbone_layerwise_0w_cap_1200_head_adam": {
        ...
        "clip_focus_parameter_names": HOTSPOT_CLIP_LAYERS,
        "description": (
            "Long confirmation best candidate: only refine the 0.weight hotspot "
            "cap from 2e3 to 1.2e3 while keeping the other hotspot caps unchanged."
        ),
    },
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Longer formal confirmation for critic_backbone layerwise hotspot caps, "
            "focused on 0.weight=1.2e3 versus the old 2e3 candidate."
        )
    )
...
def main() -> None:
    ...
    print(
        "Critic backbone layerwise cap long confirmation validation completed. "
