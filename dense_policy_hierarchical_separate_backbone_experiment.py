...
SEPARATE_BACKBONE_MODE_DEFINITIONS = {
    "hierarchical_actor_hard_offload_route": (
        "Control group: shared actor backbone, separate theta head and route head, "
        "A_theta_b / A_route_b unchanged, route surrogate uses hard offload mask."
    ),
    "hierarchical_actor_separate_theta_route_backbones": (
        "Test group: theta backbone and route backbone are separated, theta head still outputs "
        "raw_theta_b, route head still receives route-backbone features + detached theta context, "
        "A_theta_b / A_route_b unchanged, route surrogate still uses hard offload mask."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for shared hierarchical hard-mask actor vs "
            "separate theta/route backbone hierarchical hard-mask actor."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
        ...
        help="Policy / actor-structure modes to compare.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Separate Backbone Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Separate Backbone Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Separate Backbone Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Separate Backbone Mode",
        ylabel="approx_kl",
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_hierarchical_separate_backbone_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "separate_backbone_mode_definitions": SEPARATE_BACKBONE_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "separate_backbone_mode_definition": SEPARATE_BACKBONE_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "separate_backbone_mode_definitions": SEPARATE_BACKBONE_MODE_DEFINITIONS,
        ...
        manifest["runs"].append(
            {
                ...
                "separate_backbone_stats": str(run_dir / "separate_backbone_stats_by_epoch.csv"),
            }
        )

    summary_df = pd.DataFrame(all_metrics)
    summary_csv = root_dir / "policy_hierarchical_separate_backbone_summary.csv"
    summary_json = root_dir / "policy_hierarchical_separate_backbone_summary.json"
    summary_md = root_dir / "policy_hierarchical_separate_backbone_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy hierarchical separate-backbone experiment completed. "
