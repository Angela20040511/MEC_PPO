...
THETA_ROUTE_CANDIDATE_SCORE_CREDIT_MODE_DEFINITIONS = {
    "hierarchical_actor_theta_candidate_score_credit": (
        "Control group: keep theta candidate-score residual credit, keep route branch on the current true-conditional route credit path."
    ),
    "hierarchical_actor_theta_route_candidate_score_credit": (
        "Test group: keep the repaired theta candidate-score residual credit and additionally replace the route effective advantage entering PPO with candidate-score residual credit from existing per-BS path costs."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for theta-candidate baseline vs theta+route candidate-score residual credit."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_theta_route_candidate_score_credit_compare(
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_theta_route_candidate_score_credit_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "theta_route_candidate_score_credit_curve_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[("route_approx_kl", "Route Approx KL")],
        output_path=plots_dir / "route_kl_curve_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[("route_decision_agreement_ratio", "Route Decision Agreement")],
        output_path=plots_dir / "route_decision_agreement_curve_compare.png",
    )


def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_theta_route_candidate_score_credit_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "route_score_source": "route_score_vector = -block_path_cost_bs_now",
            ...
            json.dumps(
                {
                    ...
                    "theta_route_candidate_score_credit_mode_definition": (
                        THETA_ROUTE_CANDIDATE_SCORE_CREDIT_MODE_DEFINITIONS.get(
                            policy_ratio_mode,
                            "",
                        )
                    ),
                    ...
                    "theta_residual_credit_definition": (
                        "theta_score_vector = [local_score, best_offload_score]; "
                        "theta_effective_advantage = selected_score - expected_score under old theta Bernoulli policy."
                    ),
                    "route_residual_credit_definition": (
                        "route_score_vector = [score_bs1, score_bs2]; "
                        "route_effective_advantage = selected_score - expected_score under old conditional route categorical policy."
                    ),
                    "candidate_score_normalization_definition": (
                        "For each sample/block over candidate dimension only: "
        ...
        manifest["runs"].append(
            {
                ...
                "theta_route_candidate_score_credit_stats": str(
    ...
    summary_csv = root_dir / "policy_theta_route_candidate_score_credit_summary.csv"
    summary_json = root_dir / "policy_theta_route_candidate_score_credit_summary.json"
    summary_md = root_dir / "policy_theta_route_candidate_score_credit_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy theta+route candidate-score credit experiment completed. "
