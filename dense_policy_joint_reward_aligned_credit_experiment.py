...
JOINT_REWARD_ALIGNED_MODE_DEFINITIONS = {
    "hierarchical_actor_theta_route_candidate_score_credit": (
        "Control group: theta candidate-score residual credit plus route candidate-score residual credit, both still trained via layered theta/route surrogates."
    ),
    "hierarchical_actor_joint_reward_aligned_credit": (
        "Test group: keep the hierarchical actor parameterization, but replace layered effective credit with reward-aligned joint 3-action residual credit and joint hierarchical log-prob training."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for layered theta+route candidate credit vs reward-aligned joint 3-action credit."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_joint_reward_aligned_credit_compare(
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_joint_reward_aligned_credit_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "joint_reward_aligned_credit_curve_compare.png",
    )
    plot_two_panel_compare(
        mode_logs,
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_joint_reward_aligned_credit_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "patience": FIXED_PATIENCE,
            "min_delta": FIXED_MIN_DELTA,
            "mode_definitions": JOINT_REWARD_ALIGNED_MODE_DEFINITIONS,
            "joint_reference_definition": (
                "For each block and candidate local/bs1/bs2, keep other blocks fixed, deepcopy(simulator), "
                "step once, and use the realized one-step reward as the reward-aligned candidate score."
            ),
            ...
            json.dumps(
                {
                    ...
                    "mode_definition": JOINT_REWARD_ALIGNED_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    ...
                    "joint_reward_aligned_credit_definition": (
                        "joint_effective_advantage = selected_joint_score - expected_joint_score under old joint policy; "
                        "selected/expected scores come from one-step reward counterfactuals over local/bs1/bs2."
                    ),
    ...
    summary_csv = root_dir / "policy_joint_reward_aligned_credit_summary.csv"
    summary_json = root_dir / "policy_joint_reward_aligned_credit_summary.json"
    summary_md = root_dir / "policy_joint_reward_aligned_credit_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy joint reward-aligned credit experiment completed. "
