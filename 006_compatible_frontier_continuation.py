...
EXPERIMENT_ID = "006"
EXPERIMENT_STEM = "006_compatible_frontier_continuation"
...
GROUP_SPECS: tuple[dict[str, Any], ...] = (
    {
        "group_name": "006_pure_blended_baseline_adam",
        "description": (
            "Compatible default training semantics with pure blended critic baseline: "
            "main_value_loss = 0.8 * current_batch_loss + 0.2 * fixed_heldout_batch_loss."
        ),
        "soft_geometry_enabled": False,
    },
    {
        "group_name": "006_pure_blended_narrow_scope_soft_geometry_adam",
        "description": (
            "Compatible default training semantics with the retained 005 lighter narrow-scope "
            "soft geometry branch on top of the same pure blended baseline."
        ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="006 compatible-default frontier continuation on pure blended vs narrow scope."
    )
    parser.add_argument(
        "--output-root",
        type=str,
        default="D:\\MEC_PPO\\checkpoints",
        help="Directory where the numbered 006 output root will be created.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Existing 006 output root to resume from; completed runs are reused and missing runs continue in-place.",
    )
...
def _summarize_seed(seed: int, rows: list[dict[str, Any]], root_dir: Path) -> pd.DataFrame:
    ...
    (root_dir / f"006_seed_{seed}_summary.json").write_text(
        df.to_json(orient="records", force_ascii=False, indent=2),
        encoding="utf-8",
    )
    df.to_csv(root_dir / f"006_seed_{seed}_summary.csv", index=False, encoding="utf-8-sig")
    (root_dir / f"006_seed_{seed}_summary.md").write_text(
...
def _build_cross_seed_summary(all_rows: list[dict[str, Any]]) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    baseline_name = GROUP_SPECS[0]["group_name"]
    summary_rows = []
    long_rows = []
...
def _plot_narrow_vs_baseline(summary_df: pd.DataFrame, output_path: Path) -> None:
    ...
    baseline_name = GROUP_SPECS[0]["group_name"]
    narrow_name = GROUP_SPECS[1]["group_name"]
    deltas = []
    ...
    plt.title("006 Narrow Scope vs Baseline Mean Delta")
    plt.tight_layout()
...
def _run_one_group(seed: int, group_spec: dict[str, Any], seed_root: Path, device: torch.device) -> dict[str, Any]:
    ...
    np.save(run_dir / "006_probe_states.npy", probe_states)
...
def _build_manifest(root_dir: Path, all_rows: list[dict[str, Any]]) -> None:
    ...
    (root_dir / "006_artifacts_manifest.json").write_text(
...
def main() -> None:
    ...
    if args.resume_root:
        ...
        root_dir = output_root / datetime.now().strftime(
            "006_compatible_frontier_continuation_output_%Y%m%d_%H%M%S"
        )
    ...
    for seed in SEEDS:
        ...
        seed_runs_root = root_dir / f"006_seed_{seed}_runs"
        ...
        _plot_metric_curves(
            seed_rows,
            ("episode_reward",),
            root_dir / f"006_seed_{seed}_reward_curve_compare.png",
            f"seed {seed}",
        )
        _plot_metric_curves(
            seed_rows,
            ("advantage_action_alignment", "theta_advantage_alignment", "route_advantage_alignment"),
            root_dir / f"006_seed_{seed}_advantage_alignment_curve_compare.png",
            f"seed {seed}",
        )
        _plot_metric_curves(
            seed_rows,
            ("value_explained_variance", "prediction_target_corr", "prediction_std_over_target_std"),
            root_dir / f"006_seed_{seed}_critic_health_curve_compare.png",
            f"seed {seed}",
        )
        _plot_metric_curves(
            seed_rows,
            ("critic_loss_current_batch", "critic_loss_heldout_batch", "critic_step_would_reject_rate"),
            root_dir / f"006_seed_{seed}_critic_drift_compare.png",
            f"seed {seed}",
        )

    summary_df, long_df = _build_cross_seed_summary(all_rows)
    (root_dir / "006_compatible_frontier_continuation_summary.json").write_text(
        summary_df.to_json(orient="records", force_ascii=False, indent=2),
        encoding="utf-8",
    )
    summary_df.to_csv(
        root_dir / "006_compatible_frontier_continuation_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
    md_lines = [
    ...
    (root_dir / "006_compatible_frontier_continuation_summary.md").write_text(
        "\n".join(md_lines),
        encoding="utf-8",
    )
    ...
    _plot_cross_seed_mean_curve(
        all_rows,
        ("episode_reward",),
        root_dir / "006_reward_curve_compare.png",
        "cross-seed mean",
    )
    _plot_cross_seed_mean_curve(
        all_rows,
        ("advantage_action_alignment", "theta_advantage_alignment", "route_advantage_alignment"),
        root_dir / "006_advantage_alignment_curve_compare.png",
        "cross-seed mean",
    )
    _plot_cross_seed_mean_curve(
        all_rows,
        ("value_explained_variance", "prediction_target_corr", "prediction_std_over_target_std"),
        root_dir / "006_critic_health_curve_compare.png",
        "cross-seed mean",
    )
    _plot_cross_seed_mean_curve(
        all_rows,
        ("critic_loss_current_batch", "critic_loss_heldout_batch", "critic_step_would_reject_rate"),
        root_dir / "006_critic_drift_compare.png",
        "cross-seed mean",
    )
    _plot_narrow_vs_baseline(summary_df, root_dir / "006_narrow_scope_vs_baseline_compare.png")
    _plot_box_compare(summary_df, root_dir / "006_multiseed_metric_box_compare.png")
