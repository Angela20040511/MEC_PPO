...
OFFLOAD_GATE_MODE_DEFINITIONS = {
    "hierarchical_actor_theta_route": (
        "Hierarchical actor stays fixed: theta head emits raw_theta_b first, route head emits "
        "[route_logit_BS1, route_logit_BS2] from shared actor features + detached sigmoid(theta_mean) "
        "context. Theta surrogate uses A_theta_b only; route surrogate uses A_route_b only."
    ),
    "hierarchical_actor_offload_gated_route": (
        "Same hierarchical actor structure as hierarchical_actor_theta_route, but route surrogate is "
        "multiplied by route_gate_b = detach(theta_b). Theta surrogate stays unchanged; "
        "route advantage definition stays unchanged."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for hierarchical theta-route actor vs offload-gated "
            "route surrogate."
        )
    )
    parser.add_argument(
...
def plot_route_gate_compare(
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("route_gate_mean", "Route Gate Mean"),
            ("route_gate_std", "Route Gate Std"),
            ("route_gate_active_fraction", "Route Gate Active Fraction"),
            ("route_gate_min", "Route Gate Min"),
            ("route_gate_max", "Route Gate Max"),
        ],
        output_path=output_path,
    )
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Hierarchical Route Gating Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Hierarchical Route Gating Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Hierarchical Route Gating Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Hierarchical Route Gating Mode",
        ylabel="approx_kl",
    ...
    plot_route_gate_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "route_gate_curve_compare.png",
    )
    plot_logprob_scale_compare(
        mode_logs=mode_logs,
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_hierarchical_offload_gate_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "offload_gate_mode_definitions": OFFLOAD_GATE_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "offload_gate_mode_definition": OFFLOAD_GATE_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "offload_gate_mode_definitions": OFFLOAD_GATE_MODE_DEFINITIONS,
                    ...
                    "block_surrogate_definition": (
                        "0.5 * (surrogate_theta_b + route_gate_b * surrogate_route_b) "
                        "for hierarchical_actor_offload_gated_route; "
                        "0.5 * (surrogate_theta_b + surrogate_route_b) for control."
                    ),
        ...
        manifest["runs"].append(
            {
                ...
                "route_gate_stats": str(run_dir / "route_gate_stats_by_epoch.csv"),
            }
        )

    summary_df = pd.DataFrame(all_metrics)
    summary_csv = root_dir / "policy_hierarchical_offload_gate_summary.csv"
    summary_json = root_dir / "policy_hierarchical_offload_gate_summary.json"
    summary_md = root_dir / "policy_hierarchical_offload_gate_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy hierarchical offload-gate experiment completed. "
