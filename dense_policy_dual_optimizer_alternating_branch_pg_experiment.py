...
MODE_DEFINITIONS = {
    "hierarchical_actor_alternating_branch_pg": (
        "Control group: same separate theta/route backbones, hard route mask, branchwise "
        "normalization, and theta-first / route-second alternating updates, but both branch steps "
        "still share one actor Adam optimizer state."
    ),
    "hierarchical_actor_dual_optimizer_alternating_branch_pg": (
        "Test group: keep the same losses and update order, but use separate theta-Adam and "
        "route-Adam optimizer states so optimizer momentum no longer couples the two branches."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for alternating branch updates with shared vs split "
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Dual-Optimizer Alternating PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Dual-Optimizer Alternating PG Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Dual-Optimizer Alternating PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Dual-Optimizer Alternating PG Mode",
        ylabel="approx_kl",
    ...
    plot_theta_route_alignment_compare(
        mode_logs,
        plots_dir / "theta_route_alignment_curve_compare.png",
    )
    plot_theta_route_prob_gain_compare(
        mode_logs,
        plots_dir / "theta_route_prob_gain_curve_compare.png",
    )
    plot_theta_route_clip_compare(
        mode_logs,
        plots_dir / "theta_route_clip_curve_compare.png",
    )
    plot_theta_route_head_grad_compare(
        mode_logs,
        plots_dir / "theta_route_head_grad_curve_compare.png",
    )
    plot_theta_route_backbone_grad_compare(
        mode_logs,
        plots_dir / "theta_route_backbone_grad_curve_compare.png",
    )
    plot_alternating_step_kl_compare(
        mode_logs,
        plots_dir / "alternating_step_kl_curve_compare.png",
    )
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_dual_optimizer_alternating_branch_pg_experiment_%Y%m%d_%H%M%S"
    )
            ...
            json.dumps(
                {
                    ...
                    "mode_definition": MODE_DEFINITIONS.get(policy_ratio_mode, ""),
                    "mode_definitions": MODE_DEFINITIONS,
                    ...
                    "theta_optimizer_definition": (
                        "theta-only step uses theta Adam state only"
                        if policy_ratio_mode
                        == "hierarchical_actor_dual_optimizer_alternating_branch_pg"
                        else "theta-only step shares the global actor Adam state"
                    ),
                    "route_optimizer_definition": (
                        "route-only step uses route Adam state only"
                        if policy_ratio_mode
                        == "hierarchical_actor_dual_optimizer_alternating_branch_pg"
                        else "route-only step shares the global actor Adam state"
                    ),
    ...
    summary_csv = root_dir / "policy_dual_optimizer_alternating_branch_pg_summary.csv"
    summary_json = root_dir / "policy_dual_optimizer_alternating_branch_pg_summary.json"
    summary_md = root_dir / "policy_dual_optimizer_alternating_branch_pg_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy dual-optimizer alternating-branch PG experiment completed. "
