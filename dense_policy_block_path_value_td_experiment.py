...
PATH_VALUE_TD_MODE_DEFINITIONS = {
    "blockwise_action_conditioned_td_advantage_surrogate_mean": (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for scalar block bootstrap vs path-specific block bootstrap."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_block_path_value_compare(
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("block_path_value_local_mean", "Block Path Value Local Mean"),
            ("block_path_value_bs1_mean", "Block Path Value BS1 Mean"),
            ("block_path_value_bs2_mean", "Block Path Value BS2 Mean"),
            (
                "block_action_conditioned_value_now_mean",
                "Action-Conditioned Bootstrap Value Now Mean",
            ),
            (
                "block_action_conditioned_value_next_mean",
                "Action-Conditioned Bootstrap Value Next Mean",
            ),
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_block_path_value_td_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "path_value_td_mode_definitions": PATH_VALUE_TD_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "block_path_cost_definition": BLOCK_PATH_COST_DEFINITION,
                    "block_path_value_head_definition": BLOCK_PATH_VALUE_HEAD_DEFINITION,
                    "path_value_td_mode_definition": PATH_VALUE_TD_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "path_value_td_mode_definitions": PATH_VALUE_TD_MODE_DEFINITIONS,
    ...
    summary_csv = root_dir / "policy_block_path_value_td_summary.csv"
    summary_json = root_dir / "policy_block_path_value_td_summary.json"
    summary_md = root_dir / "policy_block_path_value_td_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy block path-value TD experiment completed. "
