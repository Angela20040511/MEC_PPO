...
HIERARCHICAL_ACTOR_MODE_DEFINITIONS = {
    "theta_route_split_advantage_surrogate": (
        "Flat joint actor head still emits [theta_b, route_logit_BS1, route_logit_BS2] together; "
        "loss uses A_theta_b only for theta and A_route_b only for route logits."
    ),
    "hierarchical_actor_theta_route": (
        "Shared actor backbone stays fixed; theta head emits raw_theta_b first; "
        "route head receives shared actor features + detached sigmoid(theta_mean) context, "
        "then emits [route_logit_BS1, route_logit_BS2]. "
        "A_theta_b trains theta only, A_route_b trains route only."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for flat split-loss actor vs hierarchical theta-route actor."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Actor Structure",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Actor Structure",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Actor Structure",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Actor Structure",
        ylabel="approx_kl",
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_hierarchical_actor_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "hierarchical_actor_mode_definitions": HIERARCHICAL_ACTOR_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "hierarchical_actor_mode_definition": HIERARCHICAL_ACTOR_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "hierarchical_actor_mode_definitions": HIERARCHICAL_ACTOR_MODE_DEFINITIONS,
    ...
    summary_csv = root_dir / "policy_hierarchical_actor_summary.csv"
    summary_json = root_dir / "policy_hierarchical_actor_summary.json"
    summary_md = root_dir / "policy_hierarchical_actor_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy hierarchical actor experiment completed. "
