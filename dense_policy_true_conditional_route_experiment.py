...
TRUE_CONDITIONAL_ROUTE_MODE_DEFINITIONS = {
    "hierarchical_actor_factorized_ratio_pg": (
    ...
    "hierarchical_actor_true_conditional_route_policy": (
        "Test group: keep the same structure, advantages, alternating updates, and branchwise PPO "
        "objectives, but reinterpret the actor as a true two-level conditional policy: first a "
        "Bernoulli local-vs-offload decision, then a conditional categorical BS1-vs-BS2 route policy "
        "defined only on offload-active samples."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for factorized-ratio PG vs true conditional-route policy."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_conditional_policy_active_fraction_compare(
    mode_logs: dict[str, pd.DataFrame],
    output_path: Path,
) -> None:
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("offload_active_fraction", "Offload Active Fraction"),
            ("route_logprob_active_fraction", "Route Log-Prob Active Fraction"),
            ("conditional_policy_consistency_score", "Conditional Policy Consistency"),
        ],
        output_path=output_path,
    )
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by True Conditional-Route Policy Mode",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by True Conditional-Route Policy Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by True Conditional-Route Policy Mode",
        ylabel="approx_kl",
    ...
    plot_metric_compare(
        mode_logs,
        column="route_approx_kl",
        title="Route Approx KL by True Conditional-Route Policy Mode",
        ylabel="route_approx_kl",
        output_path=plots_dir / "route_kl_curve_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="route_clip_fraction",
        title="Route Clip Fraction by True Conditional-Route Policy Mode",
        ylabel="route_clip_fraction",
        output_path=plots_dir / "route_clip_curve_compare.png",
    )
    plot_conditional_policy_active_fraction_compare(
        mode_logs=mode_logs,
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_true_conditional_route_policy_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "true_conditional_route_mode_definitions": TRUE_CONDITIONAL_ROUTE_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "true_conditional_route_mode_definition": TRUE_CONDITIONAL_ROUTE_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "true_conditional_route_mode_definitions": TRUE_CONDITIONAL_ROUTE_MODE_DEFINITIONS,
                    ...
                    "true_conditional_route_old_new_logprob_definition": (
                        "For hierarchical_actor_true_conditional_route_policy only: "
                        "theta branch uses Bernoulli(logit=theta_mean_b) over local/offload; "
                        "route branch uses Categorical(logits=[mu_bs1_b, mu_bs2_b]) over BS1/BS2, "
                        "evaluated only on offload-active samples."
                    ),
    ...
    summary_csv = root_dir / "policy_true_conditional_route_summary.csv"
    summary_json = root_dir / "policy_true_conditional_route_summary.json"
    summary_md = root_dir / "policy_true_conditional_route_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy true-conditional-route policy experiment completed. "
