...
BLOCK_ADVANTAGE_SCALE_DEFINITION = (
    "For blockwise_scaled_advantage_surrogate_mean only: "
    "per-block local_activity = workload + virtual_queue + local_queue + sensor-shared access_queue, "
    "all from the current normalized state at the same timestep; "
    "block_adv_scale = (local_activity + eps) / mean_blocks(local_activity + eps), "
    "so scales are nonnegative and each sample has mean block scale = 1."
)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Controlled dense comparison for blockwise surrogate advantage scaling."
    )
    parser.add_argument(
        "--policy-ratio-modes",
        nargs="+",
...
def plot_block_adv_scale_compare(
    ...
    plot_two_panel_compare(
        mode_logs,
        panel_specs=[
            ("block_adv_scale_mean", "Block Advantage Scale Mean"),
            ("block_adv_scale_std", "Block Advantage Scale Std"),
            ("block_adv_scale_max", "Block Advantage Scale Max"),
            ("block_adv_scale_min", "Block Advantage Scale Min"),
            ("block_adv_scale_entropy", "Block Advantage Scale Entropy"),
            ("top_k_block_adv_scale_share", "Top-K Block Advantage Scale Share"),
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_block_adv_scale_compare(
        mode_logs=mode_logs,
        output_path=plots_dir / "block_adv_scale_curve_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="block_adv_scale_entropy",
        title="Block Advantage Scale Entropy by Policy Surrogate Mode",
        ylabel="block_adv_scale_entropy",
        output_path=plots_dir / "block_adv_scale_entropy_curve_compare.png",
    )
    plot_metric_compare(
        mode_logs,
        column="top_k_block_adv_scale_share",
        title="Top-K Block Advantage Scale Share by Policy Surrogate Mode",
        ylabel="top_k_block_adv_scale_share",
        output_path=plots_dir / "top_k_block_adv_scale_share_curve_compare.png",
    )
    plot_bucket_metric_compare(
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime("dense_policy_block_advantage_experiment_%Y%m%d_%H%M%S")
    ...
    summary_csv = root_dir / "policy_block_advantage_summary.csv"
    summary_json = root_dir / "policy_block_advantage_summary.json"
    summary_md = root_dir / "policy_block_advantage_summary.md"
    ...
    manifest["summary"] = {
        ...
        "block_adv_scale_curve_compare": str(
            root_dir / "comparison_plots" / "block_adv_scale_curve_compare.png"
        ),
        "block_adv_scale_entropy_curve_compare": str(
            root_dir / "comparison_plots" / "block_adv_scale_entropy_curve_compare.png"
        ),
        "top_k_block_adv_scale_share_curve_compare": str(
            root_dir / "comparison_plots" / "top_k_block_adv_scale_share_curve_compare.png"
        ),
...
def main() -> None:
    ...
    print(f"Dense policy block advantage experiment completed. Results saved to: {root_dir}")
