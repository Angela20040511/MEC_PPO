...
FACTORZIED_RATIO_MODE_DEFINITIONS = {
    "hierarchical_actor_alternating_branch_pg": (
        "Control group: separate theta / route backbones, hard route mask, branchwise normalized "
        "advantages, and alternating theta-first then route-second updates, but overall diagnostics "
        "still inherit the current mixed blockwise summary path."
    ),
    "hierarchical_actor_factorized_ratio_pg": (
        "Test group: keep the same alternating theta-first then route-second updates, but treat "
        "theta and route as explicit PPO branches with separate old/new log-probs, ratios, clipped "
        "surrogates, KL, clip fractions, and branch-balanced overall summaries."
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Controlled dense comparison for alternating branch PG vs fully factorized branch-ratio PG."
        )
    )
    parser.add_argument(
        "--policy-ratio-modes",
...
def plot_comparisons(root_dir: Path, summary_df: pd.DataFrame) -> None:
    ...
    plot_metric_compare(
        ...
        title="Reward Curve by Factorized Ratio PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="policy_loss",
        title="Policy Loss by Factorized Ratio PG Mode",
        ylabel="policy_loss",
    ...
    plot_metric_compare(
        ...
        title="Clip Fraction by Factorized Ratio PG Mode",
    ...
    plot_metric_compare(
        mode_logs,
        column="approx_kl",
        title="Approx KL by Factorized Ratio PG Mode",
        ylabel="approx_kl",
...
def run_experiment(policy_ratio_modes: list[str], seed: int, output_root: Path) -> Path:
    root_dir = output_root / datetime.now().strftime(
        "dense_policy_factorized_ratio_pg_experiment_%Y%m%d_%H%M%S"
    )
    ...
    manifest: dict[str, object] = {
        ...
        "fixed_params": {
            ...
            "factorized_ratio_mode_definitions": FACTORZIED_RATIO_MODE_DEFINITIONS,
            ...
            json.dumps(
                {
                    ...
                    "factorized_ratio_mode_definition": FACTORZIED_RATIO_MODE_DEFINITIONS.get(
                        policy_ratio_mode,
                        "",
                    ),
                    "factorized_ratio_mode_definitions": FACTORZIED_RATIO_MODE_DEFINITIONS,
    ...
    summary_csv = root_dir / "policy_factorized_ratio_pg_summary.csv"
    summary_json = root_dir / "policy_factorized_ratio_pg_summary.json"
    summary_md = root_dir / "policy_factorized_ratio_pg_summary.md"
...
def main() -> None:
    ...
    print(
        "Dense policy factorized-ratio PG experiment completed. "
